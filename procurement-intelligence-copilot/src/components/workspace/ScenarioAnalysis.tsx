...
import { getRankedSuppliers } from '../../services/scenarioService';
import { SlidersHorizontal, RotateCcw, TrendingUp, Sparkles, ShieldCheck } from 'lucide-react';
...
export const ScenarioAnalysis: React.FC<ScenarioAnalysisProps> = ({ suppliers }) => {
  const { scenarioWeights, updateScenarioWeights, resetScenarioWeights } = useAppStore();

  const rankedSuppliers = getRankedSuppliers(suppliers, scenarioWeights);
                ...
                >
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <div className={`text-[13px] font-medium ${isFirst ? 'text-sandstone/80' : 'text-stone-gray'}`}>
                        ${supplier.rawCost.toLocaleString()} • {supplier.rawTime}d lead time
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-6 justify-between sm:justify-end border-t sm:border-t-0 pt-3 sm:pt-0 border-pure-white/10">
                    ...
                    <Badge status={isFirst ? 'PASS' : supplier.status}>
                      {isFirst ? 'Top Ranked' : supplier.status.replace('_', ' ')}
                    </Badge>
                  </div>
                </motion.div>
              );
