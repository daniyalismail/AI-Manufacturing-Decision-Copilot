...
export default function DashboardPage() {
  ...
  const { userProfile, setProjects } = useAppStore();
  ...
  const activeCount = projectsList.filter(p => p.status !== 'Draft' && p.status !== 'Analyzed').length;

  if (isLoading) {
    return (
      <div className="py-24 text-center space-y-4">
        <div className="text-[20px] font-bold text-ink-black animate-pulse">Loading dashboard...</div>
      </div>
    );
  }

  return (
    <div className="space-y-10 animate-fade-in duration-500 pt-4">
      {/* Header Banner */}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {[
          { label: 'Active Projects', val: projectsList.length.toString(), sub: `${activeCount} in progress`, icon: FileText, color: 'text-sky-pop' },
          { label: 'Suppliers Evaluated', val: '12', sub: 'across active categories', icon: ShieldCheck, color: 'text-fresh-grass' },
        ...
        <Card className="bg-ink-black text-pure-white p-8 md:p-10 relative overflow-hidden card-shadow">
          <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
            <div className="space-y-3 max-w-2xl">
              <div className="flex items-center gap-3">
                ...
                <span className="text-[13px] text-sandstone/80 font-medium">
                  {topProject.title || topProject.name}
                </span>
              </div>
            ...
            <Button
              ...
              className="gap-2 shrink-0 text-[15px] font-bold shadow-lg bg-pure-white text-ink-black hover:bg-sandstone"
            >
      ...
      <div className="space-y-6">
        <div className="flex justify-between items-center">
          ...
          <Button variant="ghost" onClick={() => router.push('/projects')} className="gap-1 text-[14px]">
            <span>View All</span>
            <ArrowRight className="w-4 h-4" />
          </Button>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {projectsList.slice(0, 4).map((proj) => (
            <Card key={proj.id} className="flex flex-col justify-between space-y-6 hover:border-ink-black/40 transition-colors cursor-pointer" onClick={() => router.push(`/projects/${proj.id}`)}>
              <div className="space-y-3">
                <div className="flex justify-between items-center">
                  <Badge status={proj.status || 'Draft'}>{proj.status || 'Draft'}</Badge>
                  <span className="text-[13px] text-stone-gray font-medium">{proj.date || new Date().toISOString().split('T')[0]}</span>
                </div>
