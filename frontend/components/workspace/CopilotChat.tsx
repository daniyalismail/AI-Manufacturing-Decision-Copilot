...
export const CopilotChat: React.FC = () => {
  ...
  const { projectChats, addChatMessage, clearChatHistory, openEvidence } = useAppStore();
  
  // Get chat history specifically for this project
  const rawChatMessages = (projectId && projectChats && projectChats[projectId]) || [];
  ...
  const handleSend = async (textToSend?: string) => {
    ...
    try {
      ...
      const aiMsg = await chatService.sendMessage(query, chatMessages, projectId || '');
      ...
      addChatMessage(projectId, {
        ...
        text: "I'm sorry, I encountered an error connecting to the intelligence engine.",
        timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
      });
    } finally {
      setIsTyping(false);
  ...
  // Find the most recent message that contains suggested questions
  const safeChatMessages = Array.isArray(chatMessages) ? chatMessages : [];
  const lastMessageWithSuggestions = [...safeChatMessages].reverse().find(msg => msg.suggestedQuestions && msg.suggestedQuestions.length > 0);
  const activeSuggested = lastMessageWithSuggestions?.suggestedQuestions || [
        ...
        {chatMessages.map((msg) => (
          ...
          >
            ...
            >
              ...
              {/* Citations Cards (Temporarily Disabled per Request) */}
