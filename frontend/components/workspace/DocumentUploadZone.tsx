  ...
  const handleFileUpload = async (files: File[]) => {
    ...
    try {
      ...
      for (const file of files) {
        ...
        try {
          const res = await uploadService.uploadDocument(projectId, file);
          ...
          uploadedDocs.push({
            ...
            status: 'Ready',
            pagesCount: 1, // backend doesn't return this yet
          });
        } catch (err) {
          console.error('Failed to upload', file.name, err);
        }
        currentProgress += (90 / files.length);
        setUploadProgress(Math.min(99, Math.round(currentProgress)));
      }
      
      const updated = [...localDocs, ...uploadedDocs];
      ...
      setTimeout(() => setIsSimulatingUpload(false), 500);
    }
  };
